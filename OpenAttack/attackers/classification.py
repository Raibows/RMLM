...
class ClassificationAttacker(Attacker):
    ...
    def __call__(self, victim: Classifier, input_: Any):
        ...
        adversarial_sample, prob = self.attack(victim, input_["x"], goal)

        return adversarial_sample, prob
        if adversarial_sample is not None:
            # todo FOR TEST THE SEVERE PROBLEM
            return adversarial_sample, prob
            y_adv = victim.get_pred([ adversarial_sample ])[0]
            if not goal.check( adversarial_sample, y_adv ):
                raise AttackException("Check attacker result failed: result ([%d] %s) expect (%s%d)" % ( y_adv, adversarial_sample, "" if goal.targeted else "not ", goal.target))
